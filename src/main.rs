...
fn function(x: f64) -> f64 {
    x.powi(2) / (9.0 - x.powi(2)).sqrt()
}

fn second_derivative(x: f64) -> f64 {
    (9.0*x.powi(2) + 162.0) / ((9.0 - x.powi(2)).sqrt() * (x.powi(4) - 18.0*x.powi(2) + 81.0))
}
...
fn get_remaining_term(
    ...
    let factor = (upper_bound - lower_bound) * step.powi(2) / 12.0;
    factor * local_max
}
